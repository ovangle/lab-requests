...
def field_of_research_division(value: FieldOfResearchDivision | FieldOfResearchGroup | FieldOfResearchCode):
    return value[:2]
...
def division_items(fields: Iterable[FieldOfResearch], division: FieldOfResearchDivision):
    yield from filter(
        lambda f: f.division == division,
        fields
    )

def group_items(fields: Iterable[FieldOfResearch], group: FieldOfResearchGroup):
    yield from filter(
        lambda f: f.group == group,
        fields
    )