...
async def index_equipments(
    ...
    return await PagedResultList[Equipment].from_selection(
        Equipment, db, query_equipments(name_istartswith=name_startswith, name_eq=name)
    )


@lab_equipments.post("/")
async def create_equipment(
    create_req: EquipmentCreate, db: LocalSession = Depends(get_db)
) -> Equipment:
    return await create_req(db)


@lab_equipments.get("/{equipment_id}")
async def read_equipment(equipment_id: UUID, db=Depends(get_db)) -> Equipment:
    return await Equipment.get_for_id(db, equipment_id)


@lab_equipments.put("/{equipment_id}")
async def update_equipment(
    equipment_id: UUID, patch: EquipmentPatch, db=Depends(get_db)
) -> Equipment:
