...
def _register_resource_views(router: APIRouter, resource_type: LabResourceType):
    ...
    @router.get("/lab/{lab_id}/" + route_prefix + "/")
    ...
    @router.get("/lab/{lab_id}" + route_prefix + "/{id}", response_model=view_type)
    ...
    @router.post("/lab/{lab_id}" + route_prefix)
...
def _register_resource_consumer_views(
    ...
    resource_prefix = "/".join(
        [
            consumer_prefix,
            "/{consumer_id}/",
            _resource_type_route_prefix(resource_type),
        ]
    )
    resources = APIRouter(prefix="/" + resource_prefix)
    router.include_router(resources)
    ...
    async def index_resources(
        self, consumer_id: UUID, db=Depends(get_db)
    ) -> LabResourceIndexPage:
    ...
    @resources.get("{resource_index}")
    async def get_resource(
        self, consumer_id: UUID, resource_index: int, db=Depends(get_db)
    ):
    ...
    async def append_resource(
        self, consumer_id: UUID, request: Request, db=Depends(get_db)
    ):
        patch = params_type.model_validate_json(await request.json())
        raise NotImplementedError()

    @resources.post("{at_index}")
    async def insert_resource(
        self, consumer_id: UUID, at_index: int, request: Request, db=Depends(get_db)
    ):
        patch = params_type.model_validate_json(await request.json())
        raise NotImplementedError()

    @resources.delete("{at_index}")
    async def delete_resource(
        self, consumer_id: UUID, at_index: int, dp=Depends(get_db)
    ):
        raise NotImplementedError()
...
def register_software_lease_consumer_views(router: APIRouter, consumer_prefix: str):
...
def register_output_material_consumer_views(router: APIRouter, consumer_prefix: str):
    return _register_resource_consumer_views(
        router,
        consumer_prefix,
...
def register_resource_consumer_views(
    ...
    if LabResourceType.EQUIPMENT_LEASE in include_types:
        register_equipment_lease_consumer_views(router, consumer_prefix)

    if LabResourceType.SOFTWARE_LEASE in include_types:
        register_software_lease_consumer_views(router, consumer_prefix)

    if LabResourceType.INPUT_MATERIAL in include_types:
        register_input_material_consumer_views(router, consumer_prefix)

    if LabResourceType.OUTPUT_MATERIAL in include_types:
        register_output_material_consumer_views(router, consumer_prefix)
