...
async def _stored_file_meta(file: AsyncBinaryIO, saved_to: Path) -> StoredFileMeta:
...
class FileStore:
    ...
    def __init__(self, path: Path | str, chunk_size: int = -1):
        from filestore import filestore_settings

        path = Path(path)
        if path.is_absolute():
            raise ValueError(
                "Configured path must be expressed relative to {root_path!s}"
            )
    ...
    async def store(
        ...
        if file.filename is None or file.content_type is None:
            raise ValueError(
                "Invalid file. Both filename and content_type must be present"
            )
