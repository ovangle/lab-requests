...
function sameValueForNewPasswordValidator(newValueAgainControl: FormControl<string>) {
...
function createCurrentPasswordValidator(resultErrors: Observable<AlterPasswordError | null>): AsyncValidatorFn {
    return async () => {
...
@Component({
    ...
    <form [formGroup]="form" (ngSubmit)="_handleSubmit($event)">
        ...
        <mat-form-field>
            ...
            <input matInput type="password" formControlName="newValueAgain" required />
...
export class AlterPasswordFormComponent {
    ...
    _clearResultSuccess() { this._resultSuccess.next(false); }
    _setResultSuccess(_: User) { this._resultSuccess.next(true); }

    readonly _resultFailure = new BehaviorSubject<AlterPasswordError | null>(null);
    _clearResultFailure() { this._resultFailure.next(null); }
    _setResultFailure(failure: AlterPasswordError) { this._resultFailure.next(failure) }
    ...
    readonly form: AlterPasswordForm = new FormGroup({
        currentValue: new FormControl<string>(
            '', 
            {
                ...
                asyncValidators: [
                    createCurrentPasswordValidator(this._resultFailure)
                ]
            }
        ),
        ...
        newValueAgain: new FormControl<string>(
            '', 
            {
                ...
                validators: [
                    Validators.required,
                    (control: AbstractControl) => sameValueForNewPasswordValidator(control as FormControl<string>)
               ]
            }
        ) 
    });
