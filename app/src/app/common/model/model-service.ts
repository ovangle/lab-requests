...
> {
    ...
    responsePageFromJson(lookup: Partial<TLookup>): (json: unknown) => ModelResponsePage<T, TLookup> {
    ...
    queryOne(lookup: Partial<TLookup>): Observable<T> {
        return this.queryPage(lookup).pipe(map(page => {
...
export function findModelService<T extends Model>(services: ModelService<any>[], model: Type<T>): ModelService<T> {
    const firstIndex = services.findIndex(item => item.model === model);
    const nextIndex = services.findIndex((item, i) => i > firstIndex && item.model === model);
    if (firstIndex < 0) {
