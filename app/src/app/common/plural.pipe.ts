import { Injectable, InjectionToken, LOCALE_ID, Pipe, PipeTransform, inject } from "@angular/core";
...
export function formatPlural(word: string, count: number, locale: string, options?: Intl.PluralRulesOptions | undefined): string {
    // save some time in the case that singular and plural are the same
    if (UNCOUNTABLE_WORDS.indexOf(word.toLowerCase()) >= 0)
        return word;
