...
import { UnitOfMeasurement, formatUnitOfMeasurement, isUnitOfMeasurement } from "src/app/common/measurement/measurement";
...
export function costEstimateFromJson(json: unknown): CostEstimate {
    ...
    const quantityRequired = typeof json['quantityRequired'] === 'number' ? json['quantityRequired'] : 1;
 ...
 * A locale code.
 */
export function formatCostEstimate(costEstimate: CostEstimate, format: 'total' | 'full', locale: string): string {
    const totalCost = formatCurrency(costEstimateTotal(costEstimate), locale, '$');
    if (format === 'total') {
        return totalCost;
    }
