...
function _collectFormControlErrors(
...
function _collectFormArrayErrors(
  arr: FormArray<any>,
): Observable<{ items: (ValidationErrors | null)[] } | null> {
  return arr.statusChanges.pipe(
    ...
    switchMap((status) => {
      ...
      const items: Observable<ValidationErrors | null>[] = arr.controls.map(
        (c) => _collectAbstractControlErrors(c),
      );
...
function _collectFormGroupErrors(
  grp: FormGroup<any>,
): Observable<ValidationErrors | null> {
...
function _collectAbstractControlErrors(
