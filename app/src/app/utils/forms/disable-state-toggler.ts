...
export function groupDisabledStateToggler<
  TControl extends { [K in keyof TControl]: AbstractControl<any> },
>(group: FormGroup<TControl>, disableControls: Array<keyof TControl>) {
  const controlTogglers = disableControls.map((k: keyof TControl) =>
    disabledStateToggler((group.controls as TControl)[k]),
  );
