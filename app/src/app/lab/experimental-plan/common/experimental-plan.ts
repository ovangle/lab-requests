...
import {
  Discipline,
  disciplineFromJson,
} from 'src/app/uni/discipline/discipline';
import {
  FundingModel,
  fundingModelFromJson,
} from 'src/app/uni/research/funding/funding-model';
...
export class ExperimentalPlan extends Model {
  ...
  get projectFiscalYearRange(): Readonly<{
    start: number;
    end: number;
  }> | null {
...
function experimentalPlanParamsFromJson(json: unknown): ExperimentalPlanParams {
  ...
  return {
    ...
    workUnits: Array.from(json['workUnits']).map((workUnit) =>
      workUnitFromJson(workUnit),
    ),
  };
}
...
export function experimentalPlanPatchToJson(patch: ExperimentalPlanPatch): {
  [k: string]: unknown;
} {
  ...
  return {
    ...
    fundingModel:
      patch.fundingModel instanceof FundingModel
        ? patch.fundingModel.id
        : patch.fundingModel,
...
export function experimentalPlanLookupToHttpParams(
  lookup: Partial<ExperimentalPlanLookup>,
): HttpParams {
...
@Injectable({ providedIn: 'root' })
export class ExperimentalPlanService extends RestfulService<
  ExperimentalPlan,
  ExperimentalPlanPatch,
  ExperimentalPlanLookup
> {
...
@Injectable({ providedIn: 'root' })
export class ExperimentalPlanCollection extends ModelCollection<
  ExperimentalPlan,
  ExperimentalPlanPatch,
  ExperimentalPlanLookup
> {
...
@Injectable()
export class ExperimentalPlanContext extends ModelContext<
  ExperimentalPlan,
  ExperimentalPlanPatch
> {
  readonly plan$ = defer(() => this.committed$);
  readonly _doUpdate = injectModelUpdate(
    ExperimentalPlanService,
    ExperimentalPlanCollection,
  );
}

export function labExperimentalPlanModelProviders() {
  return modelProviders(ExperimentalPlanMeta, ExperimentalPlanService);
}
