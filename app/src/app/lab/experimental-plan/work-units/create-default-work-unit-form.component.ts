...
import {
  WorkUnitForm,
  workUnitForm,
} from '../../work-unit/common/work-unit-form';
...
@Component({
  ...
  template: `
    <mat-checkbox [formControl]="addDefaultWorkUnitControl">
      Also create a work unit for the appropriate lab using the researcher's
      base campus and discipline
    </mat-checkbox>
...
export class ExperimentalPlanCreateDefaultWorkUnitForm {
  ...
  readonly addDefaultWorkUnitControl = new FormControl<boolean>(true, {
    nonNullable: true,
  });
  ...
  ngOnInit() {
    this._prefillFormSubscription = this.form.valueChanges
      .pipe(
        distinctUntilChanged((curr, next) => {
          return (
            curr.researcherBaseCampus === next.researcherBaseCampus &&
    ...
    this._syncDefaultWorkUnitSubscription =
      this.addDefaultWorkUnitControl.valueChanges
        .pipe(
