...
} from '../../work-unit/resource/resource-type';
import {
  CostEstimate,
...
function resourceBudgetSummaryFromJsonObject(
  json: unknown,
...
type ResourceBudgetSummaries = Partial<
  Record<ResourceType, ResourceBudgetSummary>
>;

function resourceBudgetSummariesFromJson(
  json: unknown,
): ResourceBudgetSummaries {
  if (!isJsonObject(json)) {
    throw new Error('Expected a json object');
  }
...
export interface BudgetSummaryParams extends ModelParams {
  ...
  readonly resourceSummaries: Partial<
    Record<ResourceType, ResourceBudgetSummary>
  >;
}
...
function budgetSummaryParamsFromJson(json: unknown): BudgetSummaryParams {
  ...
  return {
    ...
    planId: json['planId'],
    resourceSummaries: resourceBudgetSummariesFromJson(
      json['resourceSummaries'],
    ),
  };
}
