  ...
  template: `
    <mat-form-field>
      ...
      <mat-select multiple [formControl]="formControl" (closed)="_onTouched()">
        <mat-optgroup
          *ngFor="let group of hazardClassGroups"
          [label]="group.name"
        >
          <mat-option *ngFor="let cls of group.classes" [value]="cls">
            ...
            <div class="hazard-class-division">
              {{ _classDivisionLabel(cls) }}
            </div>
  ...
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      multi: true,
      useExisting: HazardClassesSelectComponent,
    },
  ],
})
export class HazardClassesSelectComponent implements ControlValueAccessor {
  ...
  readonly formControl = new FormControl<HazardClass[]>([], {
    nonNullable: true,
  });
