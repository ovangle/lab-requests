...
export const HAZARDOUS_MATERIAL_CLASSES: HazardClassGroup[] = [
    ...
    },
    {
        ...
        classes: [
            { id: 0, description: 'Flammable liquid'}
        ].map(v => ({groupId: 3, ...v}))
    },
    {
        id: 4,
        ...
        classes: [
            { id: 1, description: 'Flammable solid, self-reactive substances or solid desensitized explosive' },
    ...
    },
    {
        ...
        classes: [
            { id: 0, description: 'Radioactive material' }
        ].map(v => ({groupId: 7, ...v}))
    },
    {
        id: 8,
        ...
        classes: [
            { id: 0, description: 'Corrosive substance'}
        ].map(v => ({groupId: 8, ...v}))
    },
    {
        id: 9,
        ...
        classes: [
            { id: 0, description: 'Miscelaneous dangerous substance'}
        ].map(v => ({groupId: 9, ...v}))
    }
];
...
export function hazardClassGroup(cls: HazardClass): HazardClassGroup {
    ...
    if (maybeGroup?.id !== cls.groupId) {
        throw new Error(`Could not find group for hazard class '${cls.description}'`)
    }
    return maybeGroup;
}
...
function divisionIds(division: string): [number, number] {
    ...
    if (m) {
        return [m[1], m[3] || '0'].map(x => Number.parseInt(x)) as [number, number];
    }
