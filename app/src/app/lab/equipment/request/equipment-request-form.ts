...
import { Observable, OperatorFunction, Subscription, map, merge, pipe, tap, withLatestFrom } from "rxjs";
import { CostEstimateForm, costEstimateForm, costEstimatesFromForm, setCostEstimateFormValue} from "src/app/uni/research/funding/cost-estimate/cost-estimate-form.component";
...
export function equipmentRequestForm(equipmentName?: string): EquipmentRequestForm {
...
export function setEquipmentRequestFormValue(form: EquipmentRequestForm, committed: EquipmentRequest | null) {
    if (committed == null) {
        form.reset();
    } else {
...
export function equipmentRequestFromForm(form: EquipmentRequestForm): EquipmentRequest {
    if (!form.valid) {
