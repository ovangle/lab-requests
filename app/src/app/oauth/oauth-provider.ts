...
const OAUTH_PROVIDERS = ['ovangle.com', 'cqu--microsoft'];
...
export const OAUTH_PROVIDER_PARAMS = new InjectionToken<OauthProviderParams>(
  'OAUTH_PARAMS',
);
  ...
  constructor(
    ...
    for (const oauthParams of params) {
      if (this.has(oauthParams.provider)) {
        throw new Error(
          `Multiple param objects registered for provider ${oauthParams.provider}`,
        );
      }
