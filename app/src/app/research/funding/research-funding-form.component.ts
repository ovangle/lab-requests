...
import { ResearchFundingPatch } from './research-funding';
...
function patchFromFundingModelForm(
  form: FundingModelForm,
): ResearchFundingPatch {
  if (!form.valid) {
    throw new Error('Invalid form has no patch');
  }
  return form.value as ResearchFundingPatch;
}
...
export class FundingModelFormService {
  ...
  readonly patchValue$ = this.form.valueChanges.pipe(
    filter((value): value is ResearchFundingPatch => this.form.valid),
  );
